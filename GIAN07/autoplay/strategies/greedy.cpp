///
/// GreedyStrategy - 9-direction × N-frame greedy lookahead (legacy algorithm).
///
/// Preserves original AutoPlay behavior (including its known limitations such
/// as the hysteresis comparison bug at the bottom of Update); intended as a
/// baseline for comparing newer strategies.
///
...
void GreedyStrategy::SetDifficulty(int level) {
  difficulty_ = std::clamp(level, DIFFICULTY_EASY, DIFFICULTY_HARD);
}

void GreedyStrategy::Reset() {
  prev_dir_ = 0;
  frame_counter_ = 0;
  bomb_this_frame_ = false;
}

int GreedyStrategy::GetPredictFrames() const {
  switch (difficulty_) {
...
int GreedyStrategy::GetDangerThreshold() const {
  switch (difficulty_) {
  case DIFFICULTY_EASY:
    return 5;
  case DIFFICULTY_HARD:
...
int GreedyStrategy::GetBombRadius() const {
  switch (difficulty_) {
  case DIFFICULTY_EASY:
    return 80 * 64;
  case DIFFICULTY_HARD:
...
int GreedyStrategy::GetBombThreshold() const {
  switch (difficulty_) {
  case DIFFICULTY_EASY:
    return 3;
  case DIFFICULTY_HARD:
...
int GreedyStrategy::GetGrazeRange() const {
  switch (difficulty_) {
  case DIFFICULTY_EASY:
    return 75 * 64;
  case DIFFICULTY_HARD:
...
int GreedyStrategy::GetPlayerSpeed(bool focused) const {
...
int GreedyStrategy::GetStepX(int dir, int speed) {
  switch (dir) {
  case 0:
    return 0;
  case 3:
...
int GreedyStrategy::GetStepY(int dir, int speed) {
  switch (dir) {
  case 0:
    return 0;
  case 1:
...
INPUT_BITS GreedyStrategy::Update() {
...
void GreedyStrategy::BuildPredictions(
    std::vector<BulletPrediction> &predictions) {
...
int GreedyStrategy::EvaluateCandidate(
...
int GreedyStrategy::FindBestDirection(
...
GreedyStrategy::TargetPoint GreedyStrategy::FindNearestEnemy() {
...
int GreedyStrategy::DirectionToward(int player_x, int player_y,
...
bool GreedyStrategy::ShouldFocus(int best_score) {
  return best_score <= GetDangerThreshold();
}

bool GreedyStrategy::ShouldBomb(
    const std::vector<BulletPrediction> &predictions, int player_x,
    int player_y, int speed) {
...
void GreedyStrategy::SteerTowardItems(INPUT_BITS &keys) {
...
INPUT_BITS GreedyStrategy::DirectionToKeys(int dx, int dy) {
  const int dead = 64 * 2;
